    ...
    await client.fetchInvite(newInvite)
      .then(async (invite) => {
        if (invite.guild.id !== guildId) {
          return interaction.reply({ content: 'O convite não é válido para esse servidor', ephemeral: true })
        }

        await guildModel.updateOne({ _id: guildId }, { invite: invite.code });

        return interaction.reply({ content: `Convite do servidor \`${guild.name}\` alterado com sucesso! Novo convite: ${invite.url}` })
      }, () => {
        //Aqui é feita a verificação se o convite é válido, se não for, retornaremos uma mensagem de erro
        interaction.reply({ content: 'Convite inválido... Tente novamente', ephemeral: true })
      });
  }
